...
double parallel_impl(const int rank, const int size, const int nx, const int ny, 
    ...
    //TODO: Create a communicator & initialize it.
    ...
    CUDA_RT_CALL(cudaStreamCreateWithPriority(&comm_stream, cudaStreamDefault, greatestPriority));
    cudaEvent_t comm_done;
    CUDA_RT_CALL(cudaEventCreateWithFlags(&comm_done, cudaEventDisableTiming));


    cudaEvent_t reset_l2norm_done;
    CUDA_RT_CALL(cudaEventCreateWithFlags(&reset_l2norm_done, cudaEventDisableTiming));
    cudaEvent_t comm_prep_done;
    CUDA_RT_CALL(cudaEventCreateWithFlags(&comm_prep_done, cudaEventDisableTiming));
    ...
    while (l2_norm > tol && iter < iter_max) {
        ...
        CUDA_RT_CALL(cudaStreamWaitEvent(comm_stream, reset_l2norm_done, 0));
        ...
        stencil_kernel(a_new, a, l2_norm_d, (iy_end - 1), iy_end, nx, calculate_norm,
                             comm_stream);
        CUDA_RT_CALL(cudaEventRecord(comm_prep_done, comm_stream));

        if (calculate_norm) {
            //Overlapped Optimization.
            CUDA_RT_CALL(cudaStreamWaitEvent(compute_stream, comm_prep_done, 0)); 
        ...
        // TODO: Replace MPI_Sendrecv with ncclSend and ncclRecv calls.
        NCCL_CALL(ncclGroupStart());
        NCCL_CALL(ncclRecv(a_new,                     nx, NCCL_REAL_TYPE, top,    nccl_comm, comm_stream));
        NCCL_CALL(ncclSend(a_new + (iy_end - 1) * nx, nx, NCCL_REAL_TYPE, bottom, nccl_comm, comm_stream));
        NCCL_CALL(ncclRecv(a_new + (iy_end * nx),     nx, NCCL_REAL_TYPE, bottom, nccl_comm, comm_stream));
        NCCL_CALL(ncclSend(a_new + iy_start * nx,     nx, NCCL_REAL_TYPE, top,    nccl_comm, comm_stream));
        NCCL_CALL(ncclGroupEnd());
        CUDA_RT_CALL(cudaEventRecord(comm_done, comm_stream));
        ...
        CUDA_RT_CALL(cudaStreamWaitEvent(compute_stream, comm_done, 0));
        std::swap(a_new, a);
        iter++;
    }
    //CUDA_RT_CALL(cudaDeviceSynchronize());
    ...
    CUDA_RT_CALL(cudaEventDestroy(comm_done));
    CUDA_RT_CALL(cudaEventDestroy(comm_prep_done));
